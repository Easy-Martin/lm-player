class VideoEvent {
    ...
    on(eventName, handle, isCustom = false) {
      if (isCustom && this.events.eventName) {
        ...
        this.video[`on${eventName}`] && this.video.addEventListener(eventName, handle, false);
      }
    }
    emit(eventName, data) {
    ...
    off(eventName, handle, isCustom = false) {
      if (isCustom && this.events.eventName) {
        ...
        this.video[`on${eventName}`] && this.video.removeEventListener(eventName, handle, false);
      }
    }
    dispose() {
      this.video = null;
